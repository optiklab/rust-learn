use concrete_boolean::{ gen_keys, server_key::ServerKey, ciphertext::Ciphertext };
...
fn main() {
    ...
    loop {
        ...
        unsafe {
            let runtime_and_energy = time_ene();
            println!("Runtime on the Echip so far: {}s", runtime_and_energy.0);
            println!("Energy cost on the Echip so far: {}J", runtime_and_energy.1);
        }
...
impl Board {
    ...
    fn new(n_cols: usize, states: Vec<Ciphertext>) -> Board {
    ...
    fn update(&mut self, server_key: &ServerKey, zeros: &(Ciphertext, Ciphertext, Ciphertext)) {
