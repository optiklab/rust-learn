...
// Creating a reference cycle of two List values pointing to each othe
fn main() {
    ...
    tree_example_with_parent_nodes_references();
    tree_example_with_reference_counts();
}
...
// Creating a Tree Data Structure: a Node with Child Nodes, 
// where Child Node has a reference to its Parent Node, but doesn’t own its parent. 
#[derive(Debug)]
struct Node {
    value: i32,
...
fn tree_example_with_parent_nodes_references() {
    let leaf = Rc::new(Node {
        value: 3,
...
fn tree_example_with_reference_counts() {
    let leaf = Rc::new(Node {
        value: 3,
        parent: RefCell::new(Weak::new()),
        children: RefCell::new(vec![]),
